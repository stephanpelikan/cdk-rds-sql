  ...
  schema: {
    ...
    Update: async (resourceId: string, oldResourceId: string, props: SchemaProps) => {
      ...
      if (props.RoleName) {
        sql.push(
          format(
            "REVOKE ALL ON ALL TABLES IN SCHEMA %I TO %I",
            oldResourceId,
            props.RoleName
          )
        )
        sql.push(
          format("REVOKE CREATE ON SCHEMA %I TO %I", oldResourceId, props.RoleName)
        )
        sql.push(format("REVOKE ALL ON SCHEMA %I TO %I", oldResourceId, props.RoleName))
      }
    ...
    Delete: (resourceId: string, props: SchemaProps) => {
      ...
      if (props.RoleName) {
        ...
        sql.push(format("REVOKE CREATE ON SCHEMA %I TO %I", resourceId, props.RoleName))
        sql.push(format("REVOKE ALL ON SCHEMA %I TO %I", resourceId, props.RoleName))
      }
